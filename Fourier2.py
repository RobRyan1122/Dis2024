...
# Function to apply Fourier transform, enhance low frequencies, and apply inverse transform for a single channel
def enhance_low_frequencies_channel(channel, enhancement_factor):
    ...
    # Increase low frequencies by the specified enhancement factor
    rows, cols = channel.shape
    crow, ccol = rows // 2, cols // 2
    low_freq_radius = min(rows, cols) // 4  # Define the radius for low frequencies

    # Create a circular mask to isolate low frequencies
